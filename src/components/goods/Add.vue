<template>
  <div>
    ...
    <el-breadcrumb separator-class="el-icon-arrow-right">
      ...
      <el-breadcrumb-item :to="{ path: '/add' }">商品列表</el-breadcrumb-item>
    ...
    <!-- 图片预览对话框 -->
    <el-dialog title="图片预览" :visible.sync="isPreviewVisible" width="35%">
      <img :src="previewUrl" alt="" class="previewImage" />
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    // 切换tab
    beforeTabLeave(activeName, formerName) {
      if (formerName === '0' && this.addForm.goods_cat.length !== 3) {
    ...
    handlePreview(file) {
      console.log(file)
      this.previewUrl = 'https://lianghj.top:8888/' + file.response.data.tmp_path
    ...
    async add() {
      ...
      this.onlyTableData.forEach((item) => {
        const newInfo = { attr_id: item.attr_id, attr_val: item.attr_vals }
        this.addForm.attrs.push(newInfo)
      })
      // 深拷贝addform
